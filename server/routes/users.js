...
router.get("/editProductNum", (req, res, next) => {
  if(req.cookies && req.cookies.userId){
    let userId = req.cookies.userId;
    let proId = req.param('productId'); // 获取前台传过来的要删除的productId值
    let proNum = req.param('cartNum'); // 获取前台传过来的要删除的productNum值
    ...
    pool.query(sql, (err, result) => {
      if(err) {
        ...
        if(err) {
          ...
          //   data:values
          // });
          let inSql = "insert into orderlist(orderId,userId,productId,productName,productPrice,cartNum,productImg,postName,streetName,postCode,tel,totalPrice,itemPrice,discount,shipPrice,freightRisk,createDate,ifPay) values?";
