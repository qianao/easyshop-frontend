<template>
  <div>
        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList" v-if="item.shoppingCart.checked=='1'">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img :src="`../../static/${item.productImg.url}`" alt="XX">
                  </div>
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{item.shoppingCart.productPrice | formatMoney('')}}</div>
                </div>
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    <div class="select-self">
                      <div class="select-self-area">
                        <span class="select-ipt">x{{item.shoppingCart.cartNum}}</span>
                      </div>
                    </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productParams.productPrice*item.shoppingCart.cartNum | formatMoney('')}}</div>
                </div>
              </li>
            </ul>
          </div>
    ...
    <pay-suc :mdShow="mdShow" v-on:close="closeModal">
      ...
      <!--<div class="pay-sao" slot="message" v-if="0">-->
...
import {_aliPay, _deleteCartItem, _generateOrderItem, _insertOrders} from '../api/orders'
import axios from 'axios'
import {_getById} from  '../api/address'
  export default{
      data(){
          return{
            cartList:[],
        ...
        itemPrice(){
          ...
          this.cartList.forEach((item)=>{
            if(item.shoppingCart.checked=='1'){
              money += parseFloat(item.productParams.productPrice)*parseInt(item.shoppingCart.cartNum);
            }
          })
          return money;
        },
      ...
      methods: {
        ...
        twoJsonMerge(json1,json2) {
         ...
         pay() {
          var addressId = this.$route.query.addrId;
          // var address = {}
          //  _getById(addressId).then((res)=>{console.log(res);address=res.data});
          var param = {
            userId:this.userId,
            // address:address,
            addressId:Number(addressId),
            totalAmount:this.itemPrice,
            actualAmount:this.totalPrice,
            payType:2,
            status:1,
            deliverType:"中通"
          };
           // param=this.twoJsonMerge(param,address);
           // console.log(address)
           _insertOrders(param)
             .then((res) => {
            // res = res.data;
            ...
            if(res.status) {
              ...
              this.cartList.forEach((item)=>{
                if(item.shoppingCart.checked=='1'){
                  _generateOrderItem(this.orderId,item.product.productId,item.productImg.url,item.productParams.productPrice,item.shoppingCart.cartNum)
                  _deleteCartItem(item.shoppingCart.cartId)
                }
              })
              // setTimeout(()=>{
              //   this.mdShow = false;
              //
              //   this.$router.push({
              //     path:`/orderinfo?m=${Base64.encode(res.data)}&n=${Base64.encode(this.totalPrice)}`
              // });
              // }, 60000);
            }
          });
        ...
        goAlipay () {
          ...
          window.location.href=('http://127.0.0.1:8088/api/payment/pay/'+this.orderId+'/'+this.totalPrice)
          // window.location.href = "localhost:8088/api/payment/pay/29345";
        },
