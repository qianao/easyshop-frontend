<template>
  <div>
    ...
    <div class="container">
      <div class="cart" v-if="this.cartList.length">
              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check" @click="changeMoney(item,2)">
                    <a class="checkbox-btn item-check-btn" :class="{'check':item.shoppingCart.checked==='1'}">
                  ...
                  <div class="cart-item-pic">
                    <img :src="`../../static/${item.productImg.url}`">
                  </div>
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{item.productParams.weight}}</div>
                </div>
                <div class="cart-tab-2">
                  <div class="item-price">{{item.productParams.productPrice | formatMoney('')}}</div>
                </div>
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="changeMoney(item,-1)">-</a>
                        <span class="select-ipt">{{item.shoppingCart.cartNum}}</span>
                        <a class="input-add" @click="changeMoney(item,1)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.shoppingCart.cartNum*item.shoppingCart.productPrice| formatMoney('')}}</div>
                </div>
      ...
      <div class="ifnull" v-if="!this.cartList.length">
        <div class="content">
    ...
    export default{
        ...
        methods:{
          getCartNum(user_id){
            ...
            axios.get('/shoppingCart/getCartData',{
              params:param
            }).then((res) => {
              res = res.data;
          ...
          changeMoney(product, flag){
            if(flag === 1){
              product.shoppingCart.cartNum++;
            }else if(flag === -1) {
              if(product.shoppingCart.cartNum>1) {
                product.shoppingCart.cartNum--;
              }else {
                product.shoppingCart.cartNum = 1;
              }
            } else{
              product.shoppingCart.checked = product.shoppingCart.checked=='1'?'0':'1';
              console.log(product)
              this.selectedCount=product.shoppingCart.checked=='1'? this.selectedCount+1 : this.selectedCount-1;
            }
            var param = {
              cartId:product.shoppingCart.cartId,
              cartNum:product.shoppingCart.cartNum,
            };
              axios.get('/shoppingCart/editCartNum', {
                params:param
              }).then((res) => {
                res = res.data;
                if(res.status === true) {
          ...
          selectAllToggle() {
            ...
            this.cartList.forEach((item, index) => {
              item.shoppingCart.checked = flag===true?'1':'0';
              this.selectedCount++
            });
            console.log(this.cartList)
            this.totalPrice();
          ...
          totalPrice() {
            ...
            var _this = this;
            this.cartList.forEach((item, index) => {
              if(item.shoppingCart.checked == 1){
                _this.totalMoney+=item.shoppingCart.productPrice*item.shoppingCart.cartNum;
              }
          ...
          delConfirm(item){
            this.mdShow=true;
            this.curProduct=item.shoppingCart;
          },
          delProduct() {
            var param = {
              cartId:this.curProduct.cartId
            };
            console.log(param)
            axios.get('/shoppingCart/delCart', {
              params:param
            }).then((res) => {
              res = res.data;
              if(res.status === true) {
...
<style>
  ...
  @media only screen and (max-width: 480px){
    ...
    .ifnull .content{top: 40%; transform: translate(-50%, -60%);}
  }
