          ...
          <div class="navbar-right-container" style="display: flex;">
            <div class="navbar-menu-container">
              ...
              <div class="navbar-cart-container" v-on:mouseover="cartShow" v-on:mouseout="cartHidden">
                ...
                <span class="navbar-cart-count">{{cartList.length}}</span>
                <!--购物车显示块-->
                <div class="nav-user-wrapper pa active" v-show="showCart">
                  <div class="nav-user-list">
                    <div class="full" v-show="cartList.length!==0">
                      <!--购物列表-->
                              ...
                              <div class="cart-item-inner">
                                <a @click="goDetail(item)">
                                  <div class="item-thumb">
                                      <img v-lazy="`../../static/${item.productImg.url}`">
                                  </div>
                                  <div class="item-desc">
                                      <div class="cart-cell">
                                        <h4>
                                        ...
                                        <h6><span class="price-icon">¥</span><span
                                          class="price-num">{{item.productParams.productPrice}}</span><span
                                          class="item-num">x{{item.shoppingCart.cartNum}}</span>
                                        </h6>
                                      </div>
                                  </div>
                                </a>
                      ...
                      <div class="nav-cart-total">
                        <p>共<strong>{{cartList.length}}</strong> 件商品</p>
                    ...
                    <div class="cart-con" v-show="cartList.length!==0" style="height: 313px; text-align: center;">
                      <p>您的购物车竟然是空的!</p>
                    </div>
                  </div>
                </div>
            ...
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">
                  <span class="error error-show" v-show="errTip1">密码输入错误</span>
                  <span class="error error-show" v-show="errTip2">用户名不存在</span>
                  <span class="error error-show" v-show="errTip3">用户名或密码不能为空</span>
...
export default {
  ...
  data() {
    return {
      ...
      loginShow: false,
      errTip1: false,//密码不对
      errTip2: false,//用户名不存在
      errTip3: false,//其中一个为空
      userName: '',
      userPwd: '',
      nameShow: true,
      mdShow:false,
  ...
  methods: {
    ...
    login() {
      if(!this.userName || !this.userPwd) {
        ...
        axios.get('/login', {
          ...
          console.info(res);
          if(res.data.code === 0){
            if(res.data.data.password === param.password){
              this.$store.commit('updateUser', param.username);
              this.nameShow = false;
              this.ifLogin = true;
    ...
    search() {
      ...
      this.$emit("initscroll");
        ...
        PubSub.publish("goodList",this.goodsList)
    	});
    }
  },
  components: {
